<h3 align="center"> Добро пожаловать на мою страничку!  
<img src="https://github.com/blackcater/blackcater/raw/main/images/Hi.gif" height="30"/> </h3>
<br> 
Меня зовут Маша и я аналитик данных. <br> Люблю разбираться в сложных процессах, искать причины и находить закономерности. В этом мне помогают Python, SQL, таблицы и визуализиция. <p>

<p> 💻 Стек технологий: <br> 
— Python (pandas, numpy, seaborn, matplotlib); <br> 
— SQL (MySQL, PostgreSQL); <br> 
— Среда PyCharm  <br> 
— Визуализация Power BI; <br> 
— Прочее: Google Sheets, Excel (ВПР, сводные таблицы, функции), MS PowerPoint <br> <p>


<p> 💁 Актуальное резюме: ссылка <p>

<p> Ключевые навыки: <br> 
  
- <a href="https://github.com/m-yastreb/sql-practice-analysis">Получение данных с помощью запросов MySQL</a> <br> 

- Очистка и трансформация данных с помощью Python (ссылка) <br> 

Пройденный мной курсы 

| Название курса                                       | Проводившая организация                                 | Формат                     | Период обучения | Документ об окончании |
|------------------------------------------------------|---------------------------------------------------------|----------------------------|-----------------|-----------------------|
| Основы Pandas для начинающих                         | Инженер по машинному обучению Роман Медведев            | Online на платформе Stepik |    2026         |                       |
| Интерактивный тренажер по SQL                        | Дальневосточный федеральный университет                 | Online на платформе Stepik |    2026         |                       |
| Excel: основное для работы с массивами и базами данных| Профессиональный аналитик, преподаватель Никита Сергеев | Online на платформе Stepik|    2025         |                       |

<!--
Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
